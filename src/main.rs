        ...
        for cell_column in &cells_arr_copy {
            for cell in cell_column {
                ...
                if cell_position_x - 1 > -1 && cell_position_y - 1 > -1 {
                    if cells_arr_copy[(cell_position_y - 1) as usize][(cell_position_x - 1) as usize].is_fill {
                        count_near_cells += 1;
                    }
                }
    
                if cell_position_y - 1 > -1 && cell_position_x < MAX_COLUMN_COUNT as i32 {
                    if cells_arr_copy[(cell_position_y - 1) as usize][cell_position_x as usize].is_fill {
                        count_near_cells += 1;
                    }
                }
                ...
                if cell_position_x - 1 > -1 && cell_position_y < MAX_ROWS_COUNT as i32 {
                    if cells_arr_copy[cell_position_y as usize][(cell_position_x - 1) as usize].is_fill {
                        count_near_cells += 1;
                    }
                }
